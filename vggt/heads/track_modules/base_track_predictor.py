...
class BaseTrackerPredictor(nn.Module):
    ...
    def forward(self, query_points, query_index=0, fmaps=None, iters=6, return_feat=False, down_ratio=1, apply_sigmoid=True):
        """
        ...
        query_track_feat = sample_features4d(fmaps[:, query_index], coords[:, query_index])
        ...
        for _ in range(iters):
            ...
            flows = (coords - coords[:, query_index:(query_index+1)]).permute(0, 2, 1, 3).reshape(B * N, S, 2)
            ...
            sampled_pos_emb = sample_features4d(pos_embed.expand(B, -1, -1, -1), coords[:, query_index])
            ...
            # BN, S, C
            delta = rearrange(delta, " b n s d -> (b n) s d", b=B) #I think the alst dimension is a combination of xy and 128 features
            ...
            # Force query as query
            # because we assume the query points should not be changed
            coords[:, query_index] = coords_backup[:, query_index]
